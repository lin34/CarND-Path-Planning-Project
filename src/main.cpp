        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); i++)
          {
            ...
            if (d < (2 + 4 * lane + 2) && d > (2 + 4 * lane - 2))
            {
              ...
              //check each of the lanes and set a flag
            }
            //left lane check
            int buf = 15;//lane change buffer
            if (lane !=0 && d > (0)  && d < (4)){
              if ((check_car_s - car_s) > -buf && (check_car_s - car_s) < buf){
                ln0_empty = false;
              } else{
                ln0_empty = true;
              }
          ...
          //avoid rapid lane switches

          if(lane ==0 && too_close && ln1_empty){
            lane = 1;

          }
          else if(lane ==1 && too_close){
            if(ln0_empty){//only make left turns
              lane = 0;
          ...
          else if(lane==2 && too_close &&ln1_empty){
            lane = 1;
          }
